...
class BFM():
	def __init__(self, model_root='./'):
		model_path = os.path.join(model_root, 'BFM/BFM_model_front.mat')
...
def LoadExpBasis(model_root='./'):
	n_vertex = 53215
	Expbin = open(os.path.join(model_root, 'BFM/Exp_Pca.bin'),'rb')
	exp_dim = array('i')
...
def transferBFM09(model_root='./'):
	original_BFM = loadmat(os.path.join(model_root, 'BFM/01_MorphableModel.mat'))
	...
	index_exp = loadmat(os.path.join(model_root, 'BFM/BFM_front_idx.mat'))
	index_exp = index_exp['idx'].astype(np.int32) - 1 #starts from 0 (to 53215)

	index_shape = loadmat(os.path.join(model_root, 'BFM/BFM_exp_idx.mat'))
	...
	other_info = loadmat(os.path.join(model_root, 'BFM/facemodel_info.mat'))
	...
	# save our face model
	savemat(os.path.join(model_root, 'BFM/BFM_model_front.mat'),{'meanshape':meanshape,'meantex':meantex,'idBase':idBase,'exBase':exBase,'texBase':texBase,'tri':tri,'point_buf':point_buf,'tri_mask2':tri_mask2\
		,'keypoints':keypoints,'frontmask2_idx':frontmask2_idx,'skinmask':skinmask})

# load landmarks for standard face, which is used for image preprocessing
def load_lm3d(model_root='./'):

	Lm3D = loadmat(os.path.join(model_root, 'BFM/similarity_Lm3D_all.mat'))
	Lm3D = Lm3D['lm']
