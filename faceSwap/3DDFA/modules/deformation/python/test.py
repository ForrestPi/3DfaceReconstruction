...
def loadFaceMarkersTxt(FacemarkerPath):
    with open(FacemarkerPath, 'r') as f:
         kpts = f.readlines()
    kpts = [[int(item.strip())] * 2 for item in kpts]
...
def demo():
    start = time.time()
    objpath = '/mnt/mfs/yiling/GAN/Face3d/face2face/3DDFA/results/001_f301_0.obj'
    #VS, FS = loadObj(objpath)
    VS, _, FS = mesh.interact.load_obj_with_colors(objpath)
    objpath = "/mnt/mfs/yiling/GAN/Face3d/face2face/3DDFA/results/001_f192_0.obj"
    #VS2, FS2 = loadObj(objpath)
    VS2, _, FS2 = mesh.interact.load_obj_with_colors(objpath)
    target_objpath = '/mnt/mfs/yiling/GAN/Face3d/face2face/3DDFA/results/000_f1_0.obj'
    #VT, FT = loadObj(target_objpath)
    VT, colors, FT = mesh.interact.load_obj_with_colors(target_objpath)
    #FacemarkerPath = "./Face_Marker.mat"
    #marker = loadFaceMarkers(FacemarkerPath)
    kptsPath = '../cplusplus/kpt_ind.txt'
    marker = loadFaceMarkersTxt(kptsPath)
    ...
    wi = 5.0  # smooth
    wc = [1, 500, 3000, 5000]
    VS_Reg, VT_Reg = non_rigid_registration(VS, FS, VT, FT, ws, wi, wc, marker, "results/vsp2.pkl")
    corres = build_correspondence(VS_Reg, FS, VT_Reg, FT, 10, 0.05, "results/Face_ICIP_corres.pkl")
    x, nx = deformation_transfer(VS, FS, VT, FT, VS2, FS2, corres)
    end = time.time()
    print("takes time {}".format(end - start))
    writeObj("results/out.obj", x.tolist(), FT.tolist())
    """
